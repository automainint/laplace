...
auto config::scan_flag(int argc, char **argv, string_view name)
    -> bool {
  for (size_t i = 0; i < argc; i++) {
...
auto config::scan_flag(
    int argc, char **argv, char c, string_view name) -> bool {
  return scan_flag(argc, argv, c) ||
         scan_flag(argc, argv, name);
}
...
auto read_config(char **tag, char **end, ref_family cfg)
    -> size_t {
  if (tag < end && tag[0]) {
...
auto config::load(int argc, char **argv, cref_family def_cfg)
    -> family {
  family cfg = def_cfg;
