...
#define LAPLACE_GL_LOAD(a) \
    if (!gl::a) { \
    if (gl::a = reinterpret_cast<gl::pfn_##a>(gl::get_proc_address(#a));  !gl::a) { \
        cerr << "Unable to get " << #a << " function." << endl; \
        gl::ok = false; \
    } }
...
#define LAPLACE_GL_BEGIN_EX() { status = true; }

#define LAPLACE_GL_END_EX(x) { \
    if (!status) \
        cerr << #x << " OpenGL extension disabled." << endl; \
        auto i = find(gl::extensions.begin(), gl::extensions.end(), #x); \
...
void gl::require_extensions(vector<string_view> extensions)
{
    ...
    gl::extensions_required.assign(extensions.begin(), extensions.end());
}
