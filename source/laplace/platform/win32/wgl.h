...
#ifndef __laplace__platform_win32_wgl__
#define __laplace__platform_win32_wgl__
...
namespace laplace::gl {
  ...
  using pfn_wglMakeCurrent     = BOOL(__stdcall *)(HDC   hDC,
                                               HGLRC hRC);
  using pfn_wglShareLists      = BOOL(__stdcall *)(HGLRC hRC1,
                                              HGLRC hRC2);
  using pfn_wglUseFontBitmapsA = BOOL(__stdcall *)(
      HDC hDC, uint32_t first, uint32_t count, uint32_t listBase);
  using pfn_wglUseFontBitmapsW = BOOL(__stdcall *)(
      HDC hDC, uint32_t first, uint32_t count, uint32_t listBase);
  using pfn_wglUseFontOutlinesA = BOOL(__stdcall *)(
      HDC hDC, uint32_t first, uint32_t count,
      uint32_t listBase, float deviation, float extrusion,
      int32_t format, LPGLYPHMETRICSFLOAT lpgmf);
  ...
  using pfn_wglSetLayerPaletteEntries = int32_t(__stdcall *)(
      HDC hDC, int32_t iLayerPlane, int32_t iStart,
      int32_t cEntries, const uint32_t *pcr);
  using pfn_wglGetLayerPaletteEntries = int32_t(__stdcall *)(
      HDC hDC, int32_t iLayerPlane, int32_t iStart,
      int32_t cEntries, uint32_t *pcr);
  using pfn_wglRealizeLayerPalette = BOOL(__stdcall *)(
      HDC hDC, int32_t iLayerPlane, bool bRealize);
  using pfn_wglSwapLayerBuffers =
      BOOL(__stdcall *)(HDC hDC, uint32_t fuPlanes);
