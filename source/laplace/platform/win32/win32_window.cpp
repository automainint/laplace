...
void window::set_fullscreen_mode(size_t frame_width, size_t frame_height, size_t frame_rate)
{
...
void window::load_icon(HICON &icon, HICON &icon_sm)
{
    ...
    m_icon_loading = thread([this] {
        ...
        if (ExtractIconExW(m_exe_file_name.c_str(), 0, nullptr, nullptr, -1) > 0)
        {
            ExtractIconExW(m_exe_file_name.c_str(), 0, &icon, &icon_sm, 1);
            ...
            if (m_handle)
            {
                SendMessage(m_handle, WM_SETICON, ICON_BIG, reinterpret_cast<LPARAM>(icon));
                SendMessage(m_handle, WM_SETICON, ICON_SMALL, reinterpret_cast<LPARAM>(icon_sm));
            }
        }
    });
}
...
auto window::register_class() -> bool
{
    ...
    {
        cerr << __FUNCTION__ << ": RegisterClassEx failed." << endl;
        return false;
    }
...
void window::create_window()
{
    if (register_class())
    {
        ...
        {
            cerr << __FUNCTION__ << ": CreateWindowEx failed." << endl;
        }
    }
...
void window::adjust_window()
{
    ...
    AdjustWindowRectEx(&rc, static_cast<DWORD>(get_style()), false, static_cast<DWORD>(get_style_ex()));

    m_width     = static_cast<size_t>(static_cast<int64_t>(rc.right) - rc.left);
    m_height    = static_cast<size_t>(static_cast<int64_t>(rc.bottom) - rc.top);
}
...
{
    bool toggle_on = (m_handle != nullptr) && is_fullscreen() && !is_fullscreen_windowed();
    ...
    if (toggle_on)
    {
        ...
        {
            cerr << __FUNCTION__ << ": Toggle on. ChangeDisplaySettings failed." << endl;
        }
    }
    else
    {
        ...
        {
            cerr << __FUNCTION__ << ": Toggle off. ChangeDisplaySettings failed." << endl;
        }
    }
}
...
void window::accept_files(void *drop_data)
{
    ...
    if (m_on_drop_file)
    {
        ...
        {
            size_t length = DragQueryFileW(drop, static_cast<UINT>(i), nullptr, 0);
            auto file_name = make_unique<wchar_t[ ]>(length + 1);

            auto n = DragQueryFileW(drop, static_cast<UINT>(i), file_name.get(), static_cast<UINT>(length + 1));
...
auto window::process(UINT message, WPARAM wparam, LPARAM lparam) -> LRESULT
{
    LRESULT result = 0;
    ...
    default:
        result = DefWindowProcW(m_handle, message, wparam, lparam);
    }

    return result;
}

auto CALLBACK window::window_proc(HWND window_handle, UINT message, WPARAM wparam, LPARAM lparam) -> LRESULT
{
    LRESULT result = 0;

    if (message != WM_CREATE)
    {
        window *w = reinterpret_cast<window *>(GetWindowLongPtr(window_handle, GWLP_USERDATA));
...
{
    return m_fullscreen_width == 0 ? get_screen_width() : m_fullscreen_width;
}

auto window::get_fullscreen_height() const -> size_t
{
    return m_fullscreen_height == 0 ? get_screen_height() : m_fullscreen_height;
}
