...
audio::audio(audio::format request)
{
    ...
    m_sample_bytes_total = (request.channel_count * request.sample_bits) / 8;
...
void audio::buffer_adjust(size_t size_required)
{
    if (buffer_size() < size_required)
    {
        ...
        if (m_buffer.size() < size_required)
        {
            ...
            {
                vbyte temp(m_read);
                move(m_buffer.begin(), m_buffer.begin() + m_read, temp.begin());
                move(m_buffer.begin() + m_read, m_buffer.end(), m_buffer.begin());
...
void audio::buffer_write(cref_vbyte samples, size_t size)
{
    ...
    if (i1 >= m_buffer.size())
    {
        ...
        move(samples.begin(), samples.begin() + (i1 - i0), m_buffer.begin() + i0);
...
void audio::set_format(size_t channel_count, size_t sample_rate_hz, size_t sample_bits)
{
    auto _ul = unique_lock(m_lock);
...
void audio::render_buffer(uint8_t *data, size_t size)
{
    ...
    {
        copy(m_buffer.begin() + m_read, m_buffer.begin() + m_read + size, data);
...
void audio::render_thread(audio::ref a)
{
    ...
    status = CoInitializeEx(nullptr, COINIT_APARTMENTTHREADED | COINIT_SPEED_OVER_MEMORY);
    ...
    format = reinterpret_cast<WAVEFORMATEX *>(CoTaskMemAlloc(sizeof format));
    ...
    auto chunk_duration_sec = static_cast<double>(buffer_size) / format->nSamplesPerSec;
    ...
    while (!a.m_done.load(memory_order_acquire))
    {
        if (a.m_active.load(memory_order_acquire))
        {
            ...
            {
                status = render_client->GetBuffer(num_frames_available, &buffer_data);
                ...
                status = render_client->ReleaseBuffer(num_frames_available, 0);
