...
void input::set_window_rect(size_t x, size_t y, size_t width, size_t height)
{
...
void input::attach(HWND handle)
{
    if (handle)
    {
        ...
        m_keyboard_state[key_capslock].is_down      = (GetKeyState(VK_CAPITAL) & 1) == 1;
        m_keyboard_state[key_numlock].is_down       = (GetKeyState(VK_NUMLOCK) & 1) == 1;
        m_keyboard_state[key_scrolllock].is_down    = (GetKeyState(VK_SCROLL) & 1) == 1;
        ...
        {
            cerr << __FUNCTION__ << ": RegisterRawInputDevices failed." << endl;
        }
    }
}
...
{
    assert(code >= 0 && code < key_count);
    return m_keyboard_state[code].is_down && m_keyboard_state[code].is_changed;
}
...
void input::process(HRAWINPUT raw_input)
{
    ...
    auto n = GetRawInputData(raw_input, RID_INPUT, bytes.data(), &size, header_size);

    if (n == size)
    {
        auto raw = reinterpret_cast<const RAWINPUT &>(*bytes.data());
...
void input::tick(size_t delta_msec)
{
    if (m_is_char_pressed)
    {
        if (m_char_period_msec <= delta_msec)
        {
            ...
            m_char_period_msec += char_period_msec > delta_msec ? char_period_msec - delta_msec : char_period_msec;
        }
        else
        {
...
void input::process_mouse(const void *raw_data)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                m_mouse_state.cursor_x = static_cast<int>((x * m_window_width) / m_res_x);
                m_mouse_state.cursor_y = static_cast<int>((y * m_window_height) / m_res_y);
            }
        }
