...
void input::process_mouse(const void *raw_data) {
  ...
  if (m_use_system_cursor) {
    ...
    if (has(raw.usFlags, MOUSE_MOVE_ABSOLUTE)) {
      ...
      auto x =
          static_cast<int>((raw.lLastX / 65535.) * m_res_x);
      auto y =
          static_cast<int>((raw.lLastY / 65535.) * m_res_y);
      ...
      if (m_is_cursor_enabled) {
        m_mouse_state.cursor_x =
            static_cast<int>((x * m_window_width) / m_res_x);
        m_mouse_state.cursor_y =
            static_cast<int>((y * m_window_height) / m_res_y);
      }
    }
