...
void heap::read(size_t addr, size_t size, uint8_t *data)
{
    ...
    {
        cerr << __FUNCTION__ << ": Invalid address." << endl;
        return;
    }

    auto i = lower_bound(m_blocks.begin(), m_blocks.end(), n0, op_right);
    ...
    if (i != m_blocks.end())
    {
        ...
        {
            cerr << __FUNCTION__ << ": Access violation." << endl;
            return;
        }
...
void heap::write(size_t addr, size_t size, const uint8_t *data)
{
    size_t n0 = addr;
    size_t n1 = n0 + size;
...
auto heap::do_alloc(size_t size) -> size_t
{
    ...
    auto i = lower_bound(m_blocks.begin(), m_blocks.end(), offset, op_left);
...
auto heap::do_realloc(size_t offset, size_t size) -> size_t
{
    ...
    {
        if (size > i->size && !is_empty(i->offset + i->size, size - i->size))
        {
            ...
            auto j = lower_bound(m_blocks.begin(), m_blocks.end(), offset, op_left);
    ...
    {
        cerr << __FUNCTION__ << ": Invalid reallocation address." << endl;
...
void heap::do_dealloc(size_t offset)
{
    ...
    {
        cerr << __FUNCTION__ << ": Invalid deallocation address." << endl;
    }
}
...
{
    auto i = lower_bound(m_blocks.begin(), m_blocks.end(), offset, op_right);
