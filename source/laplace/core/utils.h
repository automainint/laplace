...
namespace laplace
{
    ...
    auto to_string(const char *c_format, va_list ap) -> std::string;

    auto to_u8string(const char8_t *c_format, ...) -> std::u8string;
    auto to_u8string(const char8_t *c_format, va_list ap) -> std::u8string;
    ...
    auto utf8_decode(std::u8string_view bytes, ref_uint offset, ref_char32 code) -> bool;
    auto utf8_encode(char32_t code, std::u8string &bytes, ref_uint offset) -> bool;
    ...
    constexpr auto byte_count(arg0_ arg0, args_... args) -> size_t;

    template <typename char_type, typename... args_>
    constexpr auto byte_count(std::basic_string_view<char_type> arg0, args_... args) -> size_t;

    template <typename elem_type, typename... args_>
    constexpr auto byte_count(std::span<const elem_type> arg0, args_... args) -> size_t;

    void constexpr write_bytes(std::span<uint8_t> data);

    template <laplace::trivial char_type, typename... args_>
    void constexpr write_bytes(std::span<uint8_t> data, std::basic_string_view<char_type> arg0, args_... args);

    template <laplace::trivial elem_type, typename... args_>
    void constexpr write_bytes(std::span<uint8_t> data, std::span<const elem_type> arg0, args_... args);

    template <laplace::trivial arg0_trivial_, typename... args_>
    void constexpr write_bytes(std::span<uint8_t> data, arg0_trivial_ arg0, args_... args);
