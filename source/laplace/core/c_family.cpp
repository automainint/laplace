...
auto family::get_string() const -> u8string_view
{
    ...
    return m_data.index() == n_string ? get<n_string>(m_data) : empty;
}
...
auto family::get_bytes() const -> cref_vbyte
{
    ...
    return m_data.index() == n_bytes ? get<n_bytes>(m_data) : empty;
}
...
{
    if (m_data.index() != n_composite) return;
    if (get<n_composite>(m_data).size() <= index) return;

    get<n_composite>(m_data).erase(get<n_composite>(m_data).begin() + index);
...
auto family::value(cref_family key) -> ref_family
{
    ...
    if (i == get<n_composite>(m_data).end())
    {
        return get<n_composite>(m_data).emplace_back(pair { key, family() }).second;
    }
    else if (i->first != key)
    {
        return get<n_composite>(m_data).emplace(i, pair { key, family() })->second;
    }
    else
    {
        return i->second;
...
auto family::compare(cref_family value) const -> int
{
    ...
    {
        if (m_data.index() < value.m_data.index()) return -1;
        return 1;
    }

    if (m_data.index() == n_uint)
    {
        if (get<n_uint>(m_data) == get<n_uint>(value.m_data)) return 0;
        else if (get<n_uint>(m_data) < get<n_uint>(value.m_data)) return -1;
        return 1;
    }

    if (m_data.index() == n_int)
    {
        if (get<n_int>(m_data) == get<n_int>(value.m_data)) return 0;
        else if (get<n_int>(m_data) < get<n_int>(value.m_data)) return -1;
        return 1;
    }

    if (m_data.index() == n_bool)
    {
        if (get<n_bool>(m_data) == get<n_bool>(value.m_data)) return 0;
        else if (get<n_bool>(m_data) == true) return 1;
        return -1;
    }

    if (m_data.index() == n_real)
    {
        if (get<n_real>(m_data) == get<n_real>(value.m_data)) return 0;
        else if (get<n_real>(m_data) < get<n_real>(value.m_data)) return -1;
        return 1;
    }
    ...
    if (m_data.index() == n_bytes)
    {
        ...
        {
            if (get<n_bytes>(m_data)[i] < get<n_bytes>(value.m_data)[i]) return -1;
            if (get<n_bytes>(m_data)[i] > get<n_bytes>(value.m_data)[i]) return 1;
        }
    ...
    if (m_data.index() == n_vector)
    {
        ...
        for (size_t i = 0; i < n; i++)
        {
            auto x = get<n_vector>(m_data)[i].compare(get<n_vector>(value.m_data)[i]);
    ...
    if (m_data.index() == n_composite)
    {
        ...
        for (size_t i = 0; i < n; i++)
        {
            auto x = get<n_composite>(m_data)[i].first.compare(get<n_composite>(value.m_data)[i].first);

            if (x < 0) return -1;
            if (x > 0) return 1;

            x = get<n_composite>(m_data)[i].second.compare(get<n_composite>(value.m_data)[i].second);
...
auto family::merge(cref_family f) -> bool
{
    ...
    if (f.is_composite())
    {
        for (size_t i = 0; i < f.get_size(); i++)
        {
            ...
            if (has(k))
            {
                if (get_value(k).is_composite() && f.get_value(k).is_composite())
                {
...
void family::assign(family::cref_composite value)
{
    if (value.size() > 0)
    {
        ...
        sort(get<n_composite>(m_data).begin(), get<n_composite>(m_data).end(), op_sort);
    }
    else
    {
        m_data = monostate();
...
auto family::op_lower_bound(const pair<family, family> &a, const family &key) -> bool
{
    return a.first < key;
}

auto family::op_lower_bound_uint(const pair<family, family> &a, size_t key) -> bool
{
    return a.first.is_uint() ? a.first.get_uint() < key : a.first.m_data.index() < n_uint;
}

auto family::op_sort(const pair<family, family> &a, const pair<family, family> &b) -> bool
{
