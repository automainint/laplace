...
template <>
inline auto laplace::to_u8string<>(std::string_view s)
    -> std::u8string {
  return std::u8string(s.begin(), s.end());
}

template <>
inline auto laplace::to_u8string<>(std::u8string_view s)
    -> std::u8string {
  return std::u8string(s.begin(), s.end());
}

template <typename type>
constexpr auto laplace::rd(cref_vbyte seq, size_t offset)
    -> type {
  type value = 0;
...
template <typename type>
constexpr void laplace::wr(
    uint8_t *seq, size_t offset, type value) {
  auto dst = seq + offset;
  auto src = &value;
...
constexpr auto laplace::byte_count() -> size_t { return 0; }

template <typename arg0_, typename... args_>
constexpr auto laplace::byte_count(arg0_, args_... args)
    -> size_t {
...
constexpr void laplace::write_bytes(std::span<uint8_t> data,
  ...
  if (data.size() >= size) {
    ...
    laplace::write_bytes(
        { data.data() + size, data.size() - size }, args...);
  }
}
