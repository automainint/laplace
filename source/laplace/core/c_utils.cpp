...
void laplace::error(
    std::string_view sender, string_view message) {
  auto _lck = unique_lock(g_log_mutex);
...
auto laplace::to_string(const char *c_format, va_list ap)
    -> string {
  string result;
...
auto laplace::to_u8string(const char8_t *c_format, ...)
    -> u8string {
  va_list ap;
  va_start(ap, c_format);
...
auto laplace::to_u8string(const char8_t *c_format, va_list ap)
    -> u8string {
  return as_u8string(
...
auto laplace::as_ascii_string(u8string_view s) -> string {
  return string(reinterpret_cast<const char *>(s.data()),
      reinterpret_cast<const char *>(s.data() + s.size()));
}

auto laplace::as_u8string(std::string_view s) -> u8string {
  return u8string(reinterpret_cast<const char8_t *>(s.data()),
      reinterpret_cast<const char8_t *>(s.data() + s.size()));
}

auto laplace::to_string(std::wstring_view s) -> std::string {
  std::string result;
...
auto laplace::utf8_decode(u8string_view bytes, ref_uint offset,
...
auto laplace::utf8_encode(
    char32_t code, u8string &bytes, ref_uint offset) -> bool {
  ...
  if (code <= 0x7F) {
    bytes.insert(
        bytes.begin() + offset, static_cast<uint8_t>(code));
    offset++;
    return true;
  }
  ...
  bytes.insert(
      bytes.begin() + offset, temp.begin(), temp.end());
  offset += temp.size();
