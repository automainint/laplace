/*  laplace/core/c_object_manager.cpp
 *
...
namespace laplace::core {
  ...
  void object_manager::setup(
      fn_load loader, wstring_view extension) {
    auto _ul = unique_lock(m_lock);
    m_loaders.emplace(pair { extension, loader });
  }

  void object_manager::setup(
      fn_save saver, wstring_view extension) {
    auto _ul = unique_lock(m_lock);
  ...
  void object_manager::add(ptr_pool obj_pool) {
    if (obj_pool) {
      ...
      if (find(m_pools.begin(), m_pools.end(), obj_pool) ==
          m_pools.end()) {
  ...
  void object_manager::store(
      wstring_view file_name, ptr_element obj) {
    if (obj) {
      check_pools();
  ...
  auto object_manager::load_raw(wstring_view file_name)
      -> ptr_element {
    check_pools();
  ...
  auto object_manager::save(
      wstring_view file_name, ptr_element obj) -> bool {
    bool result = false;
  ...
  void object_manager::check_pools() {
    if (has_no_pool()) {
      auto _ul = unique_lock(m_lock);
  ...
  void object_manager::locked_store(
      wstring_view file_name, ptr_element obj) {
    bool stored = false;
  ...
  auto object_manager::load_internal(wstring_view file_name)
      -> ptr_element {
    ptr_element result;
  ...
  auto object_manager::save_internal(
      wstring_view file_name, ptr_element obj) -> bool {
    bool result = false;
    auto ext    = get_ext(file_name);
  ...
  auto object_manager::get_ext(wstring_view file_name)
      -> wstring {
