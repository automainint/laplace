...
using namespace laplace::network;
using namespace std;
...
void udp_node::bind(uint16_t port)
{
    ...
    auto status = ::bind(m_socket, reinterpret_cast<const SOCKADDR *>(&name), sizeof name);

    if (status == SOCKET_ERROR)
    {
        cerr << "UDP: " << __FUNCTION__ << " - " << socket_error() << endl;
        return;
    }
    ...
    if (port == any_port)
    {
        ...
        {
            cerr << "UDP: " << __FUNCTION__ << " - getsockname failed." << endl;
            ::closesocket(m_socket);
            m_socket = INVALID_SOCKET;
            return;
        }
...
auto udp_node::receive_to(uint8_t *p, size_t count, io_mode mode) -> size_t
{
    size_t size = 0;
    ...
    if (m_socket != INVALID_SOCKET && (p != nullptr || count == 0))
    {
        ...
        for (; size < count;)
        {
            ...
            {
                cerr << "UDP: " << __FUNCTION__ << " - buffer too small (EMSGSIZE)." << endl;
                break;
            }
            ...
            else if (mode == sync)
            {
                ...
                if (!is_sync)
                {
                    cerr << "UDP: " << __FUNCTION__ << " - ioctlsocket failed." << endl;
                    break;
                }
            }
            else break;
...
auto udp_node::send_to(string_view address, uint16_t port, cref_vbyte seq) -> size_t
{
    size_t result = 0;
    ...
    if (::inet_pton(AF_INET, address.data(), &name.sin_addr.s_addr) != 1)
    {
        cerr << "UDP: " << __FUNCTION__ << " - inet_pton failed." << endl;
    }
    else
    {
...
void udp_node::init()
{
    ...
    if (m_socket == INVALID_SOCKET)
    {
        cerr << "UDP: " << __FUNCTION__ << " - socket failed." << endl;
        return;
    }
...
auto udp_node::send_internal(const sockaddr_in &name, cref_vbyte seq) -> size_t
{
    size_t count = 0;
    ...
    if (m_socket != INVALID_SOCKET)
    {
        ...
        for (; count < seq.size();)
        {
            ...
            auto n = ::sendto(m_socket, buf + count, part, 0, addr, sizeof name);
