...
void laplace::graphics::rgb_to_hsl(const float *rgb, float *hsl)
{
...
void laplace::graphics::hsl_to_rgb(const float *hsl, float *rgb)
{
    if (hsl && rgb)
    {
...
auto laplace::graphics::convert_color_channel(float x) -> uint8_t
{
    return static_cast<uint8_t>(max(0, min(static_cast<int>(x * 255 + .5f), 255)));
}

auto laplace::graphics::convert_color_channel(uint8_t x) -> float
{
    return max(0.f, min(static_cast<float>(x) / 255, 1.f));
}

void laplace::graphics::convert_rgb(const float *f_rgb, uint8_t *u8_rgb)
{
    if (f_rgb && u8_rgb)
    {
...
void laplace::graphics::convert_rgb(const uint8_t *u8_rgb, float *f_rgb)
{
    if (f_rgb && u8_rgb)
    {
...
void laplace::graphics::convert_rgba(const float *f_rgba, uint8_t *u8_rgba)
{
    if (f_rgba && u8_rgba)
    {
...
void laplace::graphics::convert_rgba(const uint8_t *u8_rgba, float *f_rgba)
{
    if (f_rgba && u8_rgba)
    {
