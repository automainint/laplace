...
void image::set_pixels(cref_vpixel pixels)
{
    ...
    copy(pixels.begin(), pixels.begin() + size, m_data.begin());
}
...
auto image::get_average(size_t x0, size_t y0, size_t x1, size_t y1, size_t z) const -> pixel
{
    pixel value;
    ...
    if (count > 0)
    {
        ...
        value.red = static_cast<uint8_t>(max(0, min(static_cast<int>(red), 255)));
        value.green = static_cast<uint8_t>(max(0, min(static_cast<int>(green), 255)));
        value.blue = static_cast<uint8_t>(max(0, min(static_cast<int>(blue), 255)));
        value.alpha = static_cast<uint8_t>(max(0, min(static_cast<int>(alpha), 255)));
    }
...
auto image::mip(size_t level, size_t z) const -> image::ptr
{
    ...
    for (size_t j = 0; j < size; j++)
    {
        ...
        {
            auto x0 = static_cast<size_t>(static_cast<double>(i * m_width) / size);
            auto y0 = static_cast<size_t>(static_cast<double>(j * m_height) / size);
            auto x1 = static_cast<size_t>(static_cast<double>((i + 1) * m_width) / size);
            auto y1 = static_cast<size_t>(static_cast<double>((j + 1) * m_height) / size);
