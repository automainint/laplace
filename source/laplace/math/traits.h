...
namespace laplace::math {
  ...
  template <typename type_>
  [[nodiscard]] constexpr auto get_size() -> sl::whole;

  template <typename type_>
  [[nodiscard]] constexpr auto get_row_count() -> sl::whole;
  ...
  &&requires(type_ v, sl::index i) {
    (void) get(v, i);
  }
  &&requires(type_ v, sl::index i, scalar_type<type_> x) {
    set(v, i, x);
  };
