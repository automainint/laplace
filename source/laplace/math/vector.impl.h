...
namespace laplace::math {
  ...
  constexpr vector<valcount, vecval>::vector(
      std::initializer_list<vecval> values) :
      v() {
  ...
  template <size_t valcount, typename vecval>
  constexpr vector<valcount, vecval>::vector(vecval value) :
      v() {
    for (size_t i = 0; i < valcount; i++) {
      this->v[i] = value;
    }
  ...
  template <size_t valcount, typename vecval>
  constexpr vector<valcount, vecval>::vector(const vecval *values) :
      v() {
    for (size_t i = 0; i < valcount; i++) {
      this->v[i] = values[i];
    }
  ...
  template <size_t valcount, typename vecval>
  inline vector<valcount, vecval> &vector<valcount, vecval>::operator+=(
  ...
  template <size_t valcount, typename vecval>
  inline vector<valcount, vecval>
      &vector<valcount, vecval>::operator*=(vecval d) {
    for (size_t i = 0; i < valcount; i++) { this->v[i] *= d; }
  ...
  template <size_t valcount, typename vecval>
  inline vecval vector<valcount, vecval>::operator[](
      size_t index) const {
    return this->v[index];
  }
  ...
  template <typename vecval>
  constexpr vector<2, vecval>::vector(
      std::initializer_list<vecval> values) :
      v() {
  ...
  template <typename vecval>
  inline vector<2, vecval> &vector<2, vecval>::operator+=(
      const vector<2, vecval> &v) {
    this->x += v.x;
    this->y += v.y;
  ...
  template <typename vecval>
  constexpr vector<3, vecval>::vector(vecval x0, vecval y0,
                                      vecval z0) :
      x(x0),
      y(y0), z(z0) { }

  template <typename vecval>
  constexpr vector<3, vecval>::vector(
      std::initializer_list<vecval> values) :
      v() {
  ...
  template <typename vecval>
  constexpr vector<3, vecval>::vector(const vector<2, vecval> &vec,
                                      vecval z0) :
      x(vec.x),
      y(vec.y), z(z0) { }
  ...
  template <typename vecval>
  inline vector<3, vecval> &vector<3, vecval>::operator+=(
  ...
  template <typename vecval>
  constexpr vector<4, vecval>::vector(vecval x0, vecval y0,
                                      vecval z0, vecval w0) :
      x(x0),
      y(y0), z(z0), w(w0) { }

  template <typename vecval>
  constexpr vector<4, vecval>::vector(
      std::initializer_list<vecval> values) :
      v() {
  ...
  template <typename vecval>
  constexpr vector<4, vecval>::vector(const vector<2, vecval> &vec,
                                      vecval z0, vecval w0) :
      x(vec.x),
      y(vec.y), z(z0), w(w0) { }

  template <typename vecval>
  constexpr vector<4, vecval>::vector(const vector<3, vecval> &vec,
                                      vecval w0) :
      x(vec.x),
      y(vec.y), z(vec.z), w(w0) { }
  ...
  template <typename vecval>
  inline vector<4, vecval> &vector<4, vecval>::operator+=(
      const vector<4, vecval> &v) {
    this->x += v.x;
    this->y += v.y;
    this->z += v.z;
    this->w += v.w;
  ...
  template <size_t valcount, typename vecval>
  inline vector<valcount, vecval> operator+(
  ...
  template <size_t valcount, typename vecval>
  inline auto operator==(const vector<valcount, vecval> &a,
                         const vector<valcount, vecval> &b)
      -> bool {
    return equals(a, b);
  }
  ...
  inline int64_t length(const vector<valcount, int64_t> &v) {
    return int64_t(
        sqrtl(static_cast<realmax_t>(square_length(v))));
  }

  template <size_t valcount, typename vecval>
  inline vecval dot(const vector<valcount, vecval> &u,
                    const vector<valcount, vecval> &v) {
    vecval d = vecval();

    for (size_t i = 0; i < valcount; i++) { d += u[i] * v[i]; }

    return d;
  }

  template <size_t valcount, typename vecval>
  inline vector<valcount, vecval> normal(
      const vector<valcount, vecval> &v) {
    return v / length(v);
  }

  template <size_t valcount, typename vecval>
  inline vector<valcount, vecval> &normalize(
      vector<valcount, vecval> &v) {
    return v /= length(v);
  }

  template <typename vecval>
  inline vector<3, vecval> cross(const vector<3, vecval> &u,
                                 const vector<3, vecval> &v) {
  ...
  template <typename vecval>
  inline vector<2, vecval> rotate(const vector<2, vecval> &v,
  ...
  template <typename vecval>
  inline vector<3, vecval> rotate(const vector<3, vecval> & v,
                                  const quaternion<vecval> &q) {
    return vector<3, vecval>(
        (q * quaternion<vecval>(v, vecval(0.0)) * conjugation(q))
            .v);
  }

  template <typename vecval>
  inline vector<4, vecval> rotate(const vector<4, vecval> & v,
                                  const quaternion<vecval> &q) {
    return vector<4, vecval>(q * quaternion<vecval>(v) *
                             conjugation(q));
  }
}
