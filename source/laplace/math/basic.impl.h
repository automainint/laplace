...
#ifndef laplace_math_basic_impl_h
#define laplace_math_basic_impl_h
...
namespace laplace::math {
  ...
  struct _equals_helper {
    ...
    static constexpr auto equals(const type_ a, const type_ b) noexcept
        -> bool {

      for (sl::index i = 0; i < get_size<type_>(); i++) {
    ...
    static constexpr auto equals(const type_ a, const type_ b,
  ...
  template <typename type_>
  requires is_scalar<type_> struct _equals_helper<type_> {
    ...
    static constexpr auto equals(const type_ a, const type_ b,
                                 const type_ epsilon) noexcept -> bool {
  ...
  template <typename type_, typename sqrt_>
  requires is_scalar<type_> struct _length_helper<type_, sqrt_> {
  ...
  template <typename type_, typename sqrt_>
  inline auto length(const type_ v, sqrt_ sqrt) -> scalar_type<type_> {

    return _length_helper<type_, sqrt_>::length(v, sqrt);
  }

  template <typename type_>
  inline auto length(const type_ v) -> scalar_type<type_> {
  ...
  struct _lerp_helper {
    ...
    static inline auto lerp(const type_ a, const type_ b,
                            const realmax_t t) {
      ...
      for (sl::index i = 0; i < get_size<type_>(); i++) {
        ...
        set(x, i,
            _lerp_helper<scalar_type<type_>>::lerp(get(a, i),
                                                   get(b, i), t));
      }
  ...
  template <typename type_>
  requires is_scalar<type_> struct _lerp_helper<type_> {

    static inline auto lerp(const type_ a, const type_ b,
                            const realmax_t t) {

      return round<type_>(std::lerp(static_cast<realmax_t>(a),
                                    static_cast<realmax_t>(b), t));
    }
  };
