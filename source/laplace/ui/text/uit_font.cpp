...
void font::print(ref_image img, int x, int y, u8string_view text)
{
    ...
    for (size_t i = 0; utf8_decode(text, i, code);)
    {
        if (m_face.load_char_render(code))
        {
            ...
            draw(img, x + slot->bitmap_left, y - slot->bitmap_top, slot->bitmap, m_color);
...
void font::draw(ref_image img, int x0, int y0, cref_bitmap bitmap, cref_pixel color)
{
    for (size_t y = 0; y < bitmap.rows; y++)
    {
        size_t row = bitmap.pitch > 0 ? y : (bitmap.rows - (y + 1));

        auto p = static_cast<const uint8_t * >(bitmap.buffer + row * abs(bitmap.pitch));
...
auto font::blend(cref_pixel s, cref_pixel d, uint8_t factor) -> pixel
{
    auto mul = [](uint8_t a, uint8_t b) -> uint8_t {
        return static_cast<uint8_t>(min(255, (static_cast<int>(a) * b + 127) / 255));
    };

    auto alpha = mul(d.alpha, factor);

    return alpha == 0 ? s : rgba(d.red, d.green, d.blue, max(s.alpha, alpha));
}
