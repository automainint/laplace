...
namespace laplace::ui::text
{
    ...
    public:
        ...
        static constexpr size_t default_scale_x     = 3;
        static constexpr size_t default_scale_y     = 3;
        static constexpr size_t default_spacing_x   = 4;
        static constexpr size_t default_spacing_y   = 4;

        ~lcd() override = default;

        void set_size(int char_top, size_t char_width, size_t char_height);
        void set_scale(size_t x, size_t y);
        void set_spacing(size_t x, size_t y);
    ...
    private:
        ...
        auto get_pixel_index(size_t x, size_t y, size_t i, size_t j) const -> size_t;

        auto get_char_top() const -> int;
        auto get_char_height() const -> size_t;

        auto get_char_left(char32_t c) const -> size_t;
        auto get_char_size(char32_t c) const -> size_t;
        auto get_char_width(char32_t c) const -> size_t;

        void draw_char(graphics::ref_image img, int x, int y, char32_t c);
        static void draw_dot(graphics::ref_image img, int x0, int y0, int x1, int y1, graphics::cref_pixel color);
        ...
        size_t          m_scale_x       = default_scale_x;
        size_t          m_scale_y       = default_scale_y;
        size_t          m_spacing_x     = default_spacing_x;
        size_t          m_spacing_y     = default_spacing_y;
        int             m_char_top      = 0;
