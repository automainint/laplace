...
void lcd::set_size(int char_top, size_t char_width, size_t char_height)
{
    m_char_top      = char_top;
...
void lcd::set_scale(size_t x, size_t y)
{
    m_scale_x = x;
    m_scale_y = y;
}

void lcd::set_spacing(size_t x, size_t y)
{
    m_spacing_x = x;
    m_spacing_y = y;
}
...
void lcd::adjust_size(size_t index, char32_t c)
{
    ...
    for (++i; i < m_char_width; i++)
    {
        ...
        if (is_empty)
        {
            m_sizes[index].size = i - m_sizes[index].left;
            break;
        }
    }
    ...
    {
        m_sizes[index].size = m_char_width - m_sizes[index].left;
    }
}
...
auto lcd::get_pixel_index(size_t x, size_t y, size_t i, size_t j) const -> size_t
{
...
{
    return static_cast<int>(m_scale_y * m_char_top);
}

auto lcd::get_char_height() const -> size_t
{
    return static_cast<size_t>(m_scale_y * m_char_height + m_spacing_y);
}
...
auto lcd::get_char_size(char32_t c) const -> size_t
{
    if (c < char_first || c >= char_first + char_count)
...
auto lcd::get_char_width(char32_t c) const -> size_t
{
...
{
    size_t i0 = get_char_x(c) + get_char_left(c);
    size_t j0 = get_char_y(c);
...
void lcd::draw_dot(ref_image img, int x0, int y0, int x1, int y1, cref_pixel color)
{
    for (int i = x0; i < x1; i++)
    {
        for (int j = y0; j < y1; j++)
