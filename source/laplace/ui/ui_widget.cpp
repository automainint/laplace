...
size_t widget::renderbuffer_count = widget::default_renderbuffer_count;

auto widget::tick(size_t delta_msec, platform::ref_input in) -> bool
{
...
void widget::draw_childs()
{
    if (m_expired_childs > 0)
    {
        ...
        for (size_t i = 0; i < m_childs.size(); i++)
        {
            ...
            if (w->m_is_visible && intersects(local_area, w->m_rect))
            {
...
auto widget::widget_tick(size_t delta_msec, platform::ref_input in) -> bool
{
    bool result = false;
...
{
    if (child->m_is_attached && child->m_parent.lock() == shared_from_this())
    {
...
void widget::refresh()
{
    m_expired = renderbuffer_count;
    m_expired_childs = m_expired;
...
void widget::refresh_childs()
{
    ...
    {
        p->m_expired_childs = renderbuffer_count;
    }
}
