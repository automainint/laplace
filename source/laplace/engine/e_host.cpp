...
void host::listen(uint16_t port) {
  ...
  perform_event(
      slot_host, server_clock(get_tick_duration()).encode());
  perform_event(slot_host,
      server_seed(get_solver()->get_seed()).encode());
}
...
void host::receive_chunks() {
  if (m_node) {
    for (;;) {
      ...
      auto plain =
          m_slots[slot].cipher.decrypt({ m_buffer.data(), n });
...
void host::send_chunks() {
  for (size_t i = 0; i < m_slots.size(); i++) {
    ...
    if (m_slots[i].encrypt) {
      auto chunk =
          m_slots[i].cipher.encrypt({ buf.data(), chunk_size });
...
auto host::perform_control(size_t slot, cref_vbyte seq)
  ...
  if (public_key::scan(seq)) {
    if (public_key::get_cipher(seq) == ids::cipher_dh_rabbit) {
      send_event_to(
          slot, public_key(ids::cipher_dh_rabbit,
  ...
  if (client_enter::scan(seq) && slot != slot_host) {
    ...
    m_slots[slot].is_ready =
        get_state() != server_state::prepare;
