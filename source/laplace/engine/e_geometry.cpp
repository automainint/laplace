...
namespace laplace::engine
{
    ...
    void append(ref_box a, cref_vec3i point)
    {
        ...
        {
            if (a.min.x > point.x) a.min.x = point.x;
            if (a.min.y > point.y) a.min.y = point.y;
            if (a.min.z > point.z) a.min.z = point.z;
            if (a.max.x < point.x) a.max.x = point.x;
            if (a.max.y < point.y) a.max.y = point.y;
            if (a.max.z < point.z) a.max.z = point.z;
        }
    }
    ...
    {
        if (index == 0) return a.min;
        if (index == 1) return vec3i(a.min.x, a.min.y, a.max.z);
        if (index == 2) return vec3i(a.min.x, a.max.y, a.min.z);
        if (index == 3) return vec3i(a.min.x, a.max.y, a.max.z);
        if (index == 4) return vec3i(a.max.x, a.min.y, a.min.z);
        if (index == 5) return vec3i(a.max.x, a.min.y, a.max.z);
        if (index == 6) return vec3i(a.max.x, a.max.y, a.min.z);
        return a.max;
    }
    ...
    {
        if (a.size() < 3) return plane { a[0], vec3i(0, 0, 0) };
        return plane { a[0], raw_normal(a[0], a[1], a[2]) };
    }

    auto plane_of(cref_box a, size_t index) -> plane
    {
        if (index == 0) return plane { a.min, vec3i(0, 0, -1) };
        if (index == 1) return plane { vec3i(a.min.x, a.min.y, a.max.z), vec3i(0, 0, 1) };
        if (index == 2) return plane { vec3i(a.min.x, a.min.y, a.max.z), vec3i(0, -1, 0) };
        if (index == 3) return plane { vec3i(a.min.x, a.max.y, a.max.z), vec3i(0, 1, 0) };
        if (index == 4) return plane { vec3i(a.min.x, a.max.y, a.max.z), vec3i(-1, 0, 0) };
    ...
    auto raw_normal(cref_vec3i p0, cref_vec3i p1, cref_vec3i p2) -> vec3i
    {
        auto a = p1 - p0;
        auto b = p0 - p2;
        return math::cross(a, b);
    ...
    auto orientation(cref_triangle plane, cref_vec3i point) -> vecval
    {
    ...
    auto square_distance(cref_triangle plane, cref_vec3i point) -> vecval
    {
        auto n = raw_normal(plane);
    ...
    auto abs_square_distance(cref_triangle plane, cref_vec3i point) -> vecval
    {
        auto n = raw_normal(plane);
        auto l = math::square_length(n);
    ...
    auto contains_flat(cref_triangle a, cref_vec3i point) -> bool
    {
        for (size_t i = 0; i < 3; i++)
        {
            ...
            if (math::dot(r0, r1) + epsilon <= 0) return false;
        }
    ...
    {
        if (auto dx = abs(point.x - a.base.x); dx - epsilon <= a.radius)
        if (auto dy = abs(point.y - a.base.y); dy - epsilon <= a.radius)
        {
            auto rr = dx * dx + dy * dy;
    ...
    {
        if (a.min.x - epsilon >= point.x) return false;
        if (a.min.y - epsilon >= point.y) return false;
        if (a.min.z - epsilon >= point.z) return false;
        if (a.max.x + epsilon <= point.x) return false;
        if (a.max.y + epsilon <= point.y) return false;
        if (a.max.z + epsilon <= point.z) return false;
        return true;
    }

    auto contains(cref_box a, cref_box b) -> bool
    {
        if (a.min.x - epsilon >= b.min.x) return false;
        if (a.min.y - epsilon >= b.min.y) return false;
        if (a.min.z - epsilon >= b.min.z) return false;
        if (a.max.x + epsilon <= b.max.x) return false;
        if (a.max.y + epsilon <= b.max.y) return false;
        if (a.max.z + epsilon <= b.max.z) return false;
        return true;
    }

    auto contains(cref_box a, cref_triangle b) -> bool
    {
        if (!contains(a, b[0])) return false;
        if (!contains(a, b[1])) return false;
        if (!contains(a, b[2])) return false;
        return true;
    }
    ...
    {
        if (auto dx = abs(point.x - a.center.x); dx - epsilon <= a.radius)
        if (auto dy = abs(point.y - a.center.y); dy - epsilon <= a.radius)
        if (auto dz = abs(point.z - a.center.z); dz - epsilon <= a.radius)
        {
    ...
    {
        if (!contains(a.bounds, point)) return false;
        if (a.childs.index() == 0)
        {
            auto &childs = get<0>(a.childs);
            for (auto &c : childs) if (contains(c, point)) return true;
            return false;
        }
        else
        {
            auto &childs = get<1>(a.childs);
            for (auto &tr : childs) if (orientation(tr, point) - epsilon >= 0) return false;
        }
        return true;
    }
    ...
    {
        if (a.max.x + epsilon <= b.min.x) return false;
        if (a.max.y + epsilon <= b.min.y) return false;
        if (a.max.z + epsilon <= b.min.z) return false;
        if (b.max.x + epsilon <= a.min.x) return false;
        if (b.max.y + epsilon <= a.min.y) return false;
        if (b.max.z + epsilon <= a.min.z) return false;
        return true;
    }

    auto intersects(cref_box a, cref_cylinder b) -> bool
    {
        auto contains_flat = [](vecval x, vecval y, const cylinder &b) -> bool {
        ...
        if (a.max.z + epsilon <= b.base.z) return false;
        if (a.min.z - epsilon >= b.base.z + b.height) return false;
        if (contains_flat(a.min.x, a.min.y, b)) return true;
        if (contains_flat(a.min.x, a.max.y, b)) return true;
        if (contains_flat(a.max.x, a.min.y, b)) return true;
        if (contains_flat(a.max.x, a.max.y, b)) return true;
        return false;
    }

    auto intersects(cref_box a, cref_sphere b) -> bool
    {
        if (a.max.x + epsilon <= b.center.x - b.radius) return false;
        if (a.max.y + epsilon <= b.center.y - b.radius) return false;
        if (a.max.z + epsilon <= b.center.z - b.radius) return false;
        if (a.min.x - epsilon >= b.center.x + b.radius) return false;
        if (a.min.y - epsilon >= b.center.y + b.radius) return false;
        if (a.min.z - epsilon >= b.center.z + b.radius) return false;
        for (size_t i = 0; i < 8; i++) if (contains(b, vertex_of(a, i))) return true;
        return false;
    }
    ...
    auto intersects(cref_box a, cref_triangle b) -> bool
    {
        ...
        if (!intersects(a, bounds)) return false;

        auto v0_ori = orientation(b, vertex_of(a, 0));
        if (abs(v0_ori) <= epsilon) return true;

        for (size_t i = 1; i < 8; i++)
        {
            auto v_ori = orientation(b, vertex_of(a, i));
            if (abs(v_ori) <= epsilon) return true;
            if ((v0_ori > 0) != (v_ori > 0)) return true;
        }
    ...
    {
        if (!intersects(a, b.bounds)) return false;
        if (b.childs.index() == 0)
        {
            for (auto &c : get<0>(b.childs))
            {
                if (intersects(a, c)) return true;
            }
        }
        else
        {
            ...
            {
                if (intersects(a, tr)) return true;
            }
        }
        return false;
    }
    ...
    {
        if (a.base.z - epsilon >= b.base.z + b.height) return false;
        if (a.base.z + a.height + epsilon <= b.base.z) return false;
    ...
    {
        if (a.base.z - epsilon >= b.center.z + b.radius) return false;
        if (a.base.z + a.height + epsilon <= b.center.z - b.radius) return false;
        
        vecval dx, dy;
        auto d = a.radius + b.radius;
        if (dx = abs(a.base.x - b.center.x); dx - epsilon >= d) return false;
        if (dy = abs(a.base.y - b.center.y); dy - epsilon >= d) return false;
        auto r = dx * dx + dy * dy;
        if (r - epsilon >= d) return false;
        
        auto l = static_cast<vecval>(sqrtl(static_cast<long double>(r)) - static_cast<long double>(a.radius) + .5);
        auto h = a.radius * a.radius - l * l;
        if (a.base.z - epsilon >= b.center.z + h) return false;
        if (a.base.z + a.height + epsilon <= b.center.z - h) return false;
        return true;
    }
    ...
    auto intersects(cref_cylinder a, cref_triangle b) -> bool
    {
        ...
        if (contains(a, b[1])) return true;
        if (contains(a, b[2])) return true;
        ...
        if (square_distance(stem, ray { b[0], b[1] - b[0] }) - epsilon <= d) return true;
        if (square_distance(stem, ray { b[1], b[2] - b[1] }) - epsilon <= d) return true;
        if (square_distance(stem, ray { b[2], b[0] - b[2] }) - epsilon <= d) return true;
    ...
    {
        if (!intersects(b.bounds, a)) return false;
        if (b.childs.index() == 0)
        {
            for (auto &c : get<0>(b.childs))
            {
    ...
    auto intersects(cref_sphere a, cref_triangle b) -> bool
    {
        ...
        if (contains(a, b[1])) return true;
        if (contains(a, b[2])) return true;
        return abs_square_distance(b, a.center) - epsilon <= a.radius * a.radius;
    }
    ...
    auto intersects(cref_triangle a, cref_triangle b) -> bool
    {
        ...
        if (abs(ori0) >= epsilon && abs(ori1) >= epsilon && abs(ori2) >= epsilon)
        {
            if ((ori0 < 0) == (ori1 < 0))
            if ((ori0 < 0) == (ori2 < 0))
            return false;
        ...
        if (contains_flat(a, b[0])) return true;
        if (contains_flat(a, b[1])) return true;
        if (contains_flat(a, b[2])) return true;
        if (contains_flat(b, a[0])) return true;
        if (contains_flat(b, a[1])) return true;
        if (contains_flat(b, a[2])) return true;
    ...
    auto intersects(cref_octree a, cref_octree b) -> bool
    {
        ...
        {
            auto &childs = get<0>(a.childs);
            for (auto &c : childs) if (intersects(c, b)) return true;
        }
        else if (b.childs.index() == 0)
        {
            auto &childs = get<0>(b.childs);
            for (auto &c : childs) if (intersects(a, c)) return true;
        }
        else
        {
            ...
            for (auto &tr0 : v0)
            {
                ...
                {
                    for (auto &tr1 : v1) if (intersects(tr0, tr1)) return true;
                }
            }
        }
        return false;
    ...
    {
        if (contains(b, a.base)) return true;

        auto t0 = min(intersection(a, plane_of(b, 0)), intersection(a, plane_of(b, 1)));
        auto t1 = min(intersection(a, plane_of(b, 2)), intersection(a, plane_of(b, 3)));
        auto t2 = min(intersection(a, plane_of(b, 4)), intersection(a, plane_of(b, 5)));
    ...
    auto intersects(cref_ray ra, cref_cylinder cyl) -> bool
    {
        ...
        if (square_distance(ra, ray { cyl.base, vec3i { 0, 0, 1 } }) + epsilon >= rr)
        {
            return false;
        }

        auto t0 = intersection(ra, plane {
            cyl.base,
            vec3i { 0, 0, 1 }
        });

        if (t0 + epsilon >= 0 && contains_flat(cyl, point_of(ra, t0)))
        {
            return true;
        }

        auto t1 = intersection(ra, plane {
            vec3i { cyl.base.x, cyl.base.y, cyl.base.z + cyl.height },
            vec3i { 0, 0, 1 }
        });

        if (t1 + epsilon >= 0 && contains_flat(cyl, point_of(ra, t1)))
        {
            return true;
        }
    ...
    {
        return square_distance(ra, sph.center) - epsilon <= sph.radius * sph.radius;
    }
    ...
    auto intersection(cref_ray a, cref_box b) -> vecval
    {
        ...
        {
            auto t0 = max(intersection(a, plane_of(b, 0)), intersection(a, plane_of(b, 1)));
            auto t1 = max(intersection(a, plane_of(b, 2)), intersection(a, plane_of(b, 3)));
            auto t2 = max(intersection(a, plane_of(b, 4)), intersection(a, plane_of(b, 5)));
     ...
     *      a x^2 + b x + c = 0
     */
    static auto quadratic_solution(vecval a, vecval b, vecval c) -> vecval
    {
        if (a == 0)
        {
            /*  Linear solution.
        ...
        auto delta = static_cast<vecval>(sqrtl(static_cast<long double>(d)));

        return (b <= 0 && -b >= delta ? -b - delta : -b + delta) / (2 * a);
    }
    ...
    auto intersection(cref_ray ra, cref_cylinder cyl) -> vecval
    {
         ...
         *
         *      (vx^2 + vy^2) t^2 + 2 (vx dx + vy dy) t + (dx^2 + dy^2 - r^2) = 0
         */

        auto t = quadratic_solution(
            vx * vx + vy * vy,
            2 * (vx * dx + vy * dy),
            dx * dx + dy * dy - rr
        );
        ...
        if (t > -safe_limit)
        {
            ...
            if (z >= cyl.base.z - epsilon || z <= cyl.base.z + cyl.height + epsilon)
            {
                return t;
            }
            ...
            if (t0 > t1)
            {
                if (contains_p0) return t0;
                if (contains_p1) return t1;
            }
            else
            {
                if (contains_p1) return t1;
    ...
    auto intersection(cref_ray ra, cref_sphere sph) -> vecval
    {
         ...
         *  Intersection condition:
         *
         *      (vx t + x0 - cx)^2 + (vy t + y0 - cy)^2 + (vz t + z0 - cz)^2 - r^2 = 0
         */
        ...
        /*  Intersection condition:
         *
         *      (vx t + dx)^2 + (vy t + dy)^2 + (vz t + dz)^2 - r^2 = 0
         *
         *      (vx^2 + vy^2 + vz^2) t^2 + 2 (vx dx + vy dy + vz dz) t + (dx^2 + dy^2 + dz^2 - r^2) = 0
         */
