...
namespace laplace::engine {
  ...
  void basic_entity::reset_clock() {
    auto _ul    = unique_lock(m_lock);
    auto period = static_cast<uint64_t>(
        m_sets[n_tick_period].value);
    m_clock = period - 1;
  }
  ...
  void basic_entity::set_bounds(cref_box val) {
    ...
    m_sets[n_bounds_min_y].delta += val.min.y -
                                    m_sets[n_bounds_min_y].value;
    m_sets[n_bounds_min_z].delta += val.min.z -
                                    m_sets[n_bounds_min_z].value;
    m_sets[n_bounds_max_x].delta += val.max.x -
                                    m_sets[n_bounds_max_x].value;
    m_sets[n_bounds_max_y].delta += val.max.y -
                                    m_sets[n_bounds_max_y].value;
    m_sets[n_bounds_max_z].delta += val.max.z -
