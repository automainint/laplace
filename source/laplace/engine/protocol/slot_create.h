...
namespace laplace::engine::protocol
{
    ...
    public:
        ...
        constexpr slot_create(size_t index, uint64_t time, size_t id_actor, bool is_local)
        {
            set_order({ index });
            set_time(time);
            set_actor(id_actor);
         ...
         */
        static constexpr void alter(std::span<uint8_t> seq, size_t id_slot_actor)
        {
            wr<uint8_t>(seq, n_local_flag, (get_slot_actor(seq) == id_slot_actor));
        }
