...
namespace laplace::engine {
  ...
  void dh_rabbit::set_remote_key(cref_vbyte key) {
    ...
    auto status =
        wc_DhAgree(&m_key, m_mutual.data(), &m_mutual_size,
                   m_private.data(), m_private_size, key.data(),
                   key.size());
    ...
    status = wc_RabbitSetKey(&m_encrypt, m_mutual.data(),
                             m_mutual.data() + 16);
  ...
  auto dh_rabbit::encrypt(cref_vbyte bytes) -> vbyte {
    ...
    auto status = wc_RabbitProcess(&m_encrypt, result.data(),
  ...
  auto dh_rabbit::decrypt(cref_vbyte bytes) -> vbyte {
    ...
    auto status = wc_RabbitProcess(&m_decrypt, result.data(),
                                   bytes.data(), bytes.size());
