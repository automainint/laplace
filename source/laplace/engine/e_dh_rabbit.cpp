...
void dh_rabbit::init() {
  ...
  auto status = wc_DhSetKey(&m_key, protocol::prime,
      sizeof protocol::prime, protocol::generator,
      sizeof protocol::generator);
...
dh_rabbit::dh_rabbit() {
  ...
  if (m_is_ok) {
    auto status = wc_DhGenerateKeyPair(&m_key, &m_random,
        m_private.data(), &m_private_size, m_public.data(),
        &m_public_size);
...
void dh_rabbit::set_remote_key(cref_vbyte key) {
  ...
  auto status = wc_DhAgree(&m_key, m_mutual.data(),
      &m_mutual_size, m_private.data(), m_private_size,
      key.data(), key.size());
  ...
  status = wc_RabbitSetKey(
      &m_dh_rabbit, m_mutual.data(), m_mutual.data() + 16);
...
auto dh_rabbit::encrypt(cref_vbyte bytes) -> vbyte {
  ...
  auto status = wc_RabbitProcess(
      &m_dh_rabbit, result.data(), bytes.data(), bytes.size());
