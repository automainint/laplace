...
void remote::perform_queue()
{
    ...
    {
        if (m_queue.events[n].empty()) break;
...
void remote::process_buffer()
{
    ...
    while (m_client_buffer.size() - index >= 4)
    {
        ...
        if (m_client_buffer.size() - index < size + 2) break;

        add_event({
            m_client_buffer.data() + index + 2,
            size
        });
    ...
    m_client_buffer.erase(m_client_buffer.begin(), m_client_buffer.begin() + index);
}
