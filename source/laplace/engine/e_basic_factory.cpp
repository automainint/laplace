...
auto basic_factory::print_multi(
    span<const cref_vbyte> seqs) const -> string {
  string cmds;
...
auto basic_factory::parse_native(
    cref_vstring table, string_view command) -> vbyte {
  ...
  if (id == ids::undefined) {
    error(__FUNCTION__, "Unknown command (line %dz, col %dz).",
        lin, col);
    return {};
  }
  ...
  while (in.parse(" %X ", &x)) {
    if (x != (x & 0xff)) {
      error(__FUNCTION__,
          "Invalid syntax. Byte expected (line %dz, col %dz).",
          lin, col);
      return {};
    }
...
auto basic_factory::print_native(
    cref_vstring table, cref_vbyte seq) -> string {
